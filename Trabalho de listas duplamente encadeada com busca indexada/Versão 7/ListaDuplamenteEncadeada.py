...
class ListaDuplamenteEncadeada:
    ...
    def removeValor(self, valor):
        ...
        remove = self.pesquisaValores(valor)  # Pesquisa se o valor existe
        if remove is None:
            return "\nO valor não existe!\n"
        while remove is not None:
            self.indice = None
            if remove.dado == valor:
                if remove.anterior is None:  # Remove no inicio
                    self.head = remove.proximo
                    remove.proximo.anterior = None
                    self.tamanho -= 1
                elif remove == self.tail:  # Remove no fim
                    ...
                    self.tamanho -= 1
                print("Valor removido: ", remove.dado)  # Mostra o valor removido

...
class ListaDuplamenteEncadeada:
    ...
    def insert(self, valor):
        ...
        self.indice = None  # Anula a lista de indices.
        if self.head is None:  # Testa se há valores adicionados no nó
            self.head = self.tail = No(valor)
            self.tamanho += 1
            return
        inicio = self.head
        if valor <= inicio.dado:  # Insere no inicio da lista
            ...
            while inicio is not None:
                ...
                if inicio == self.tail:  # Insere no fim da lista
                    ...
                    self.tamanho += 1
                    break
                elif valor <= dado_salvo.dado:  # Insere no meio da lista
    ...
    def removeValor(self, valor):
        ...
        remove = self.pesquisaValores(valor)  # Pesquisa se o valor existe na lista
        if remove is None:  # Retorar uma string se o valor não existe na lista.
            return "\nO valor não existe!\n"
        while remove is not None:
            self.indice = None  # Anula a lista de indices.
            if remove.dado == valor:
                if remove.anterior is None:  # Remove no inicio da lista
                    self.head = remove.proximo
                    remove.proximo.anterior = None
                    self.tamanho -= 1
                elif remove == self.tail:  # Remove no fim da lista
                    self.tail = self.tail.anterior
                    self.tail.proximo = None
                    remove = self.tail.anterior
                    self.tamanho -= 1
                else:  # Remove no meio da lista
                    remove.anterior.proximo = remove.proximo
                    remove.proximo.anterior = remove.anterior
                    self.tamanho -= 1
                print("Valor removido: ", remove.dado)  # Mostra o valor removido ao usuario.
            remove = remove.proximo

    def criaIndice(self):
        """Cria um indice na lista encadeada para facilitar a procura."""
        lista_obj = self.head  # Adiciona o head da lista em uma variavel
        x = 0
        while x < 10:  # Verifica se está pulando de 10 em 10
            ...
            while i < 10:  # Verifica se esta pulando de 1 em 1
                ...
                i += 1
            self.insereValoresIndice(lista_obj)  # Adicionou o numero na lista de indices
            x += 1
    ...
    def insereValoresIndice(self, valor):
        ...
        else:  # Adiciona os numeros no tail da lista de indices
        ...
        while indiceIndex is not None:  # Testa se a lista de indices não e none
            ...
            if not(valor >= self.head.dado and valor <= self.tail.dado):  # Testa se o .dado esta na lista duplamente encadiada
                ...
                while indiceLista.dado > valor:  # Testa se o .dado da lista de indices e maior que o valor
                    indiceLista = indiceLista.anterior # Se não e maior ele volta um no (anterior) e testa novamente
                ...
                if indiceLista.dado == valor:  # Testa se o .dado e igual ao valor
                    ...
                    return None
                break
            indiceIndex = indiceIndex.proximo  # Passa para o proximo valor contido na lista de indice
    ...
    def imprimeListaDuplamenteEncadeada_crescente(self):
        ...
        print("\nO head é: ", self.primeiroElemento().dado)
        print("O tail é: ", self.ultimoElemento().dado)
